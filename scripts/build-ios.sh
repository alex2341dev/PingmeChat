...
[ -n "${PINGMECHAT_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${PINGMECHAT_ORIG_GROUP}.app/group.${PINGMECHAT_NEW_GROUP}.app/g" "ios/PingmeChat Share/PingmeChat Share.entitlements"
	sed -i "" "s/group.${PINGMECHAT_ORIG_GROUP}.app/group.${PINGMECHAT_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${PINGMECHAT_ORIG_GROUP}.app/group.${PINGMECHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${PINGMECHAT_ORIG_GROUP}.app/${PINGMECHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${PINGMECHAT_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${PINGMECHAT_ORIG_TEAM}/${PINGMECHAT_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for pingmechat.app and pingmechat.app.PingmeChat-Share)
...
[ -n "${PINGMECHAT_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/pingmechat.ipa"
  rm -rf "${TMPDIR}"
}
